...
from rovodev.commands.auth.command import app as auth_command
...
app = typer.Typer(
    help="Atlassian Rovo Dev CLI",
    pretty_exceptions_show_locals=False,
    pretty_exceptions_enable=False,
    add_completion=False,
)
app.add_typer(nautilus_app, name="nautilus", help="Nautilus MCP Server CLI", hidden=True)
app.add_typer(auth_command, name="auth")  # Mock command for auth, to populate help text only
...
def cli(version: bool | None = typer.Option(None, "--version", help="Show the Rovo Dev CLI version.", is_eager=True)):
    if version:
...
commands = {"nautilus", "run", "auth", "config", "log", "mcp"}
if IS_INTERNAL_USER:
    commands.add("serve")
